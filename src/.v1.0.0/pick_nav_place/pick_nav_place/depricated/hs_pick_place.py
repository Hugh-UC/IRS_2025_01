...
from rclpy.action.client import ClientGoalHandle, ActionClientFuture
...
class WarehouseArmController(Node):
    """
    ...
    def __init__(self):
        ...
        """
        super().__init__('warehouse_arm_controller')
    ...
    def _send_and_wait(self) -> bool:
        ...
        send_future : ActionClientFuture    = self._client.send_goal_async(goal)
        ...
        res_future : ActionClientFuture = handle.get_result_async()
...
def main(args=None):
    """
    Initializes the ROS 2 node and spins the WarehouseArmController.
    """
    rclpy.init(args=args)

    node = WarehouseArmController()
    
    try:
        # Spin the node to keep the ActionClient alive and ready for calls
        node.get_logger().info('WarehouseArmController is now spinning and ready to accept commands!')
        rclpy.spin(node)
