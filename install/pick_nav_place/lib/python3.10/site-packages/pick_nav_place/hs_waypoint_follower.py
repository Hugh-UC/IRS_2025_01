...
class WarehouseWaypointFollower(Node):
    """
    ...
    def __init__(self):
        ...
        """
        super().__init__('warehouse_waypoint_follower')
    ...
    def go_to_conveyor(self) -> bool:
        ...
        wp : PoseStamped = self._make_pose(5.81, 14.0, 0.0)
...
def main(args=None):
    """
    Initializes the ROS 2 node and spins the WarehouseWaypointFollower.
    """
    rclpy.init(args=args)
    
    node = WarehouseWaypointFollower()

    node.go_to_conveyor()
    
    try:
        # Spin the node to keep the ActionClient alive and ready for calls
        node.get_logger().info('WarehouseWaypointFollower is now spinning and ready to accept commands!')
        rclpy.spin(node)
